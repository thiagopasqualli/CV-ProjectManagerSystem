...
namespace ProjectManagerSystem.EntityFramework
{
    public class PMSDataContext : DbContext
    {
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Projeto>().HasMany(x => x.Ocorrencias).
                WithRequired(x => x.Projeto).HasForeignKey(x => x.IdProjeto);
            modelBuilder.Entity<Usuario>().HasMany(x => x.Projetos).
                WithRequired(x => x.Usuarios).HasForeignKey(x => x.IdUsuario);
